...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *UartHandle)
{
    ...
    if ((abs(ctl.rc.ch1) > 660) || \
            ...
            (abs(ctl.wheel) > 660))
    {
        ctl.rc.ch1 = ctl.rc.ch2 = ctl.rc.ch3 = ctl.rc.ch4 = ctl.wheel = 0;
        return;
    }
    ...
    Target[3] = (float)(ctl.rc.ch4 * 200 / 660);
    Vx = -Target[3];
    Target[2] = (float)(ctl.rc.ch3 * 200 / 660);
    Vy = Target[2];
    Target[0] = ctl.wheel * 800 / 660;
    Om = Target[0];
    Target[3] = (float)(ctl.rc.ch1 * 30 / 660);
    YAW = -Target[3];
    Target[4] = (int16_t)(ctl.rc.ch2 * 100 / 660);
    Pitch = Target[4];
    sw[0] = ctl.rc.sw1;
    sw[1] = ctl.rc.sw2;
...
{
    uint16_t motor_2006_data = (sw0 << 8) | sw1;
    return motor_2006_data;
}
